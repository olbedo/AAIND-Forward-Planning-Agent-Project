...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        return (self.children[actionA] & set([~l for l in self.children[actionB]])) != set()
    ...
    def _interference(self, actionA, actionB):
        ...
        """
        # TODO: implement this function
        return (self.parents[actionA] & set([~l for l in self.children[actionB]])) != set() \
             or (self.parents[actionB] & set([~l for l in self.children[actionA]])) != set()
...
class PlanningGraph:
    ...
    def _level_costs(self):
        ...
        if not self.goal - reached_goals:
            return costs
        while not self._is_leveled:
            self._extend()
            layer += 1
            for goal in self.goal - reached_goals:
                if goal in self.literal_layers[layer]:
                    costs.append(layer)
                    reached_goals.add(goal)
            if not self.goal - reached_goals:
                 return costs
    ...
    def h_setlevel(self):
        ...
        def mutually_exclusive_goals(layer):
            goals = list(self.goal)
            for i in range(len(goals)-1):
                for j in range(i + 1, len(goals)):
                    if self.literal_layers[layer].is_mutex(goals[i], goals[j]):
                        return True
            return False

        layer = 0
        if self.goal.issubset(self.literal_layers[layer]) \
                and not mutually_exclusive_goals(layer):
            return 0
        while not self._is_leveled:
            self._extend()
            layer += 1
